...
class Lap_skpm extends CI_Controller {

	/**
	* @author : Hartanto Kurniawan,S.Kom
	* @keterangan : Controller untuk halaman Laporan Surat Keterangan Persetujuan Mempelai
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Laporan Surat Keterangan Persetujuan Mempelai";
			
			$d['content'] = $this->load->view('lap_skpm/form', $d, true);		
	...
	public function lihat()
	{
		...
		if(!empty($cek)){
			...
			if($pilih=='all'){
				...
				$where = " WHERE nik1='$nik' OR nik2='$nik'";
			}
			
			$text = "SELECT * FROM skpm $where 
					ORDER BY nmr_surat ASC ";
			$total = "SELECT COUNT(*) 'tgl_pengajuan' FROM skpm $where";
			...
			$this->load->view('lap_skpm/view',$d);
		}else{
			header('location:'.base_url());
		}
	}
	public function cetak()
	{
		...
		if(!empty($cek)){
			...
			if($pilih=='all'){
				...
				$where = " WHERE nik1='$kode' OR nik2='$kode'";
				$d['filter']=" NIK $kode";
			}
			...
			$this->load->view('lap_skpm/cetak',$d);
		}else{
			header('location:'.base_url());
		}
	}
...
/* End of file lap_skpm.php */
/* Location: ./application/controllers/lap_skpm.php */