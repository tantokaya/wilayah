...
class Skau extends CI_Controller {

	/**
	 * @author : Hartanto Kurniawan,S.Kom
	 * @keterangan : Controller untuk halaman skau
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if(empty($cari)){
				...
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skau_nama LIKE '%$cari%'";
			}
			...
			$d['judul']="Surat Keterangan Asal Usul";
			...
			$text = "SELECT * FROM skau $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/skau/index/';
			...
			$d['content'] = $this->load->view('skau/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Surat Keterangan Asal Usul";
			$kode	= $this->app_model->MaxKodeskau();
			...
			$d['content'] = $this->load->view('skau/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Asal Usul";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skau WHERE nmr_surat='$id'";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skau WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skau'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skau",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['skau_nama1']		= $this->input->post('skau_nama1');
				...
				$up['skau_nama2']		= $this->input->post('skau_nama2');
				...
				$up['skau_nama3']		= $this->input->post('skau_nama3');
				...
				$data = $this->app_model->getSelectedData("skau",$id);
				if($data->num_rows()>0){
					$this->app_model->updateData("skau",$up,$id);
					echo 'Update data Sukses';
				}else{
					$this->app_model->insertData("skau",$up);
