...
class Skpnh extends CI_Controller {

	/**
	 * @author : Hartanto Kurniawan,S.Kom
	 * @keterangan : Controller untuk halaman skpnh
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skpnh_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Pengantar Naik Haji";
			...
			$text = "SELECT * FROM skpnh $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/skpnh/index/';
			...
			$d['content'] = $this->load->view('skpnh/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Tambah Surat Keterangan Pengantar Naik Haji";
			...
			$d['content'] = $this->load->view('skpnh/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Pengantar Naik Haji";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skpnh WHERE nmr_surat='$id'";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skpnh WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skpnh'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skpnh",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		$cek = $this->session->userdata('logged_in');
		if(!empty($cek)){
				$kode	= $this->app_model->MaxKodeskpnh();
				$up['nmr_surat']		= $kode;
				...
				$up['skpnh_nama']		= $this->input->post('skpnh_nama');
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("skpnh",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;
				}else{
					$kode	= $this->app_model->MaxKodeskpnh();
				$up['nmr_surat'] = $kode;
					$this->app_model->insertData("skpnh",$up);
