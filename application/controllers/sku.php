...
class Sku extends CI_Controller {
	 ...
	 * @keterangan : Controller untuk halaman sku
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR sku_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Usaha";
			...
			$config['base_url'] = site_url() . '/sku/index/';
			...
			$d['content'] = $this->load->view('sku/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Tambah Surat Keterangan Usaha";
			...
			$d['content'] = $this->load->view('sku/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Usaha";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM sku WHERE nmr_surat='$id'";
			...
			if($data->num_rows() > 0){
				foreach($data->result() as $db){
					...
					$d['jns_usaha']		=$db->jns_usaha;
					$d['lok_usaha']		=$db->lok_usaha;
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM sku WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/sku'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function berlaku()
	{
		...
		if(!empty($cek)){			
			...
			$data['berlaku'] = $this->app_model->get_berlaku_sku($tgl_berlaku);
	...
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("sku",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['sku_nama']			= $this->input->post('sku_nama');
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("sku",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;	
				}else{
					$kode	= $this->app_model->MaxKodeSku();
					
					$up['nmr_surat'] = $kode;
					$this->app_model->insertData("sku",$up);
