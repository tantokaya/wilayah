...
class Skbmr extends CI_Controller {

	/**
	 * @author : Hartanto Kurniawan,S.Kom
	 * @keterangan : Controller untuk halaman skbmr
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skbmr_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Belum Memiliki Rumah";
			...
			$config['base_url'] = site_url() . '/skbmr/index/';
			...
			$d['content'] = $this->load->view('skbmr/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('skbmr/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Belum Memiliki Rumah";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skbmr WHERE nmr_surat='$id'";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skbmr WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skbmr'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skbmr",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['skbmr_nama']		= $this->input->post('skbmr_nama');
                ...
                $up['nm_surat']		    = 'Surat Keterangan Blm Memiliki Rumah';
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("skbmr",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;	
				}else{
					$kode	= $this->app_model->MaxKodeskbmr();

				$up['nmr_surat']		= $kode;
					$this->app_model->insertData("skbmr",$up);
