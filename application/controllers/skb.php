...
class Skb extends CI_Controller {

	/**
	 * @author : Hartanto Kurniawan,S.Kom
	 * @keterangan : Controller untuk halaman skb
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skb_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Untuk Bepergian";
			...
			$text = "SELECT * FROM skb $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/skb/index/';
			...
			$d['content'] = $this->load->view('skb/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Tambah Surat Keterangan Untuk Bepergian";
			...
			$d['content'] = $this->load->view('skb/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Untuk Bepergian";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skb WHERE nmr_surat='$id'";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skb WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skb'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skb",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['skb_nama']			= $this->input->post('skb_nama');
				...
				$up['turut']			= $this->input->post('turut');
				$up['barang']			= $this->input->post('barang');
				$up['kendaraan']		= $this->input->post('kendaraan');
                ...
                $up['nm_surat']		    = 'Surat Keterangan Bepergian';
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("skb",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;	
				}else{
				$kode	= $this->app_model->MaxKodeSkb();				
				$up['nmr_surat']		= $kode;
					$this->app_model->insertData("skb",$up);
