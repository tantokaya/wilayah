...
class Lap_skau extends CI_Controller {

	/**
	* @author : Hartanto Kurniawan,S.Kom
	* @keterangan : Controller untuk halaman Laporan Surat Keterangan Asal Usul
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Laporan Surat Keterangan Asal Usul";
			
			$d['content'] = $this->load->view('lap_skau/form', $d, true);		
	...
	public function lihat()
	{
		...
		if(!empty($cek)){
			...
			if($pilih=='all'){
				...
				$where = " WHERE nik1='$nik' OR nik2='$nik' OR nik3='$nik'";
			}
			
			$text = "SELECT * FROM skau $where 
					ORDER BY nmr_surat ASC ";
			$total = "SELECT COUNT(*) 'tgl_pengajuan' FROM skau $where";
			...
			$this->load->view('lap_skau/view',$d);
		}else{
			header('location:'.base_url());
		}
	}
	public function cetak()
	{
		...
		if(!empty($cek)){
			...
			if($pilih=='all'){
				...
				$where = " WHERE nik1='$kode' OR nik2='$kode' OR nik3='$kode'";
				$d['filter']=" NIK $kode";
			}
			...
			$this->load->view('lap_skau/cetak',$d);
		}else{
			header('location:'.base_url());
		}
	}
...
/* End of file lap_skau.php */
/* Location: ./application/controllers/lap_skau.php */