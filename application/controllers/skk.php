...
class Skk extends CI_Controller {
	 ...
	 * @keterangan : Controller untuk halaman skk
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skk_nama1 LIKE '%$cari%' OR nik1 LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Kematian";
			...
			$text = "SELECT * FROM skk $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/skk/index/';
			...
			$d['content'] = $this->load->view('skk/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Tambah Surat Keterangan Kematian";
			...
			$d['content'] = $this->load->view('skk/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Kematian";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skk WHERE nmr_surat='$id'";
			...
			if($data->num_rows() > 0){
				foreach($data->result() as $db){
					...
					$d['nik']				=$db->nik1;
					$d['no_kk1']			=$db->no_kk1;
					$d['skk_nama1']			=$db->skk_nama1;
					$d['tmp_lahir1']		=$db->tmp_lahir1;
					$d['tgl_lahir1']		=$this->app_model->tgl_sql($db->tgl_lahir1);
					$d['jk1']				=$db->jk1;
					$d['kew_id1']			=$db->kew_id1;
					$d['tgl_meninggal']		=$this->app_model->tgl_sql($db->tgl_meninggal);
					$d['sebab_meninggal']	=$db->sebab_meninggal;
					$d['tmp_meninggal']		=$db->tmp_meninggal;
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skk WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skk'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skk",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['skk_nama1']		= $this->input->post('skk_nama1');
				...
				$up['tgl_meninggal']	= $this->app_model->tgl_sql($this->input->post('tgl_meninggal'));
				$up['sebab_meninggal']	= $this->input->post('sebab_meninggal');
				$up['tmp_meninggal']	= $this->input->post('tmp_meninggal');
				...
				$up['skk_nama2']		= $this->input->post('skk_nama2');
                ...
                $up['nm_surat']		    = 'Surat Keterangan Kematian';
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("skk",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;	
				}else{
				$kode	= $this->app_model->MaxKodeskk();
				$up['nmr_surat']		= $kode;
					$this->app_model->insertData("skk",$up);
