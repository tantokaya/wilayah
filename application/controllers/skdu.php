...
class Skdu extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skdu_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Domisili Usaha";
			...
			$text = "SELECT * FROM skdu $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/skdu/index/';
			...
			$d['content'] = $this->load->view('skdu/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Tambah Surat Keterangan Domilisi Usaha";
			...
			$d['content'] = $this->load->view('skdu/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Domisili Usaha";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skdu WHERE nmr_surat='$id'";
			...
			if($data->num_rows() > 0){
				foreach($data->result() as $db){
					...
					$d['nik']			=$db->nik;
					$d['no_kk']			=$db->no_kk;
					$d['skdu_nama']		=$db->skdu_nama;
					$d['kota_lahir']	=$db->kota_lahir;
					...
					$d['perusahaan']	=$db->perusahaan;
					$d['bid_usaha']		=$db->bid_usaha;
					$d['bdn_hukum']		=$db->bdn_hukum;
					$d['ak_pen']		=$db->ak_pen;
					$d['alamat_usaha']	=$db->alamat_usaha;
					$d['ttd_jenis']		=$db->ttd_jenis;
					$d['ttd_kode']		=$db->ttd_kode;
					$d['tgl_pengajuan']	=$this->app_model->tgl_str($db->tgl_pengajuan);
					$d['tgl_berlaku']	=$this->app_model->tgl_str($db->tgl_berlaku);
				}
			}else{
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skdu WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skdu'>";	
		}else{
			header('location:'.base_url());
		}
	}
	public function berlaku()
	{
		...
		if(!empty($cek)){			
			...
			$data['berlaku'] = $this->app_model->get_berlaku_skdu($tgl_berlaku);
	...
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skdu",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['skdu_nama']		=$this->input->post('skdu_nama');
				$up['kota_lahir']		=$this->input->post('kota_lahir');
				...
				$up['perusahaan']		=$this->input->post('perusahaan');
				$up['bid_usaha']		=$this->input->post('bid_usaha');
				$up['bdn_hukum']		=$this->input->post('bdn_hukum');
				$up['ak_pen']			=$this->input->post('ak_pen');
				$up['alamat_usaha']		=$this->input->post('alamat_usaha');
				$up['ttd_jenis']		=$this->input->post('ttd_jenis');
				$up['ttd_kode']			=$this->input->post('ttd_kode');
				$up['nm_surat']		    ='Surat Keterangan Domisili Usaha';
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("skdu",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;	
				}else{
					$kode	= $this->app_model->MaxKodeSkdu();
					$up['nmr_surat'] =$kode;
					$this->app_model->insertData("skdu",$up);
