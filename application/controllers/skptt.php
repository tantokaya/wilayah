...
class Skptt extends CI_Controller {

	/**
	 * @author : Hartanto Kurniawan,S.Kom
	 * @keterangan : Controller untuk halaman skptt
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skptt_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Penghasilan Tidak Tetap";
			...
			$text = "SELECT * FROM skptt $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/skptt/index/';
			...
			$d['content'] = $this->load->view('skptt/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			//$kode	= $this->app_model->MaxKodeskptt();
			...
			$d['content'] = $this->load->view('skptt/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Penghasilan Tidak Tetap";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skptt WHERE nmr_surat='$id'";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skptt WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skptt'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skptt",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['skptt_nama']		= $this->input->post('skptt_nama');
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("skptt",$up,$id);
				...
				$up['nmr_surat']		= $kode;
					$this->app_model->insertData("skptt",$up);
