...
class Skbm extends CI_Controller {

	/**
	 * @author : Hartanto Kurniawan,S.Kom
	 * @keterangan : Controller untuk halaman skbm
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR skbm_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Belum Pernah Menikah";
			...
			$text = "SELECT * FROM skbm $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/skbm/index/';
			...
			$d['content'] = $this->load->view('skbm/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Tambah Surat Keterangan Belum Pernah Menikah";
			...
			$d['content'] = $this->load->view('skbm/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Belum Pernah Menikah";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM skbm WHERE nmr_surat='$id'";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM skbm WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/skbm'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("skbm",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		$cek = $this->session->userdata('logged_in');
		if(!empty($cek)){
				$kode	= $this->app_model->MaxKodeSkbm();
				...
				$up['skbm_nama']		= $this->input->post('skbm_nama');
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("skbm",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;	
				}else{
					$kode	= $this->app_model->MaxKodeSkbm();
					$up['nmr_surat'] = $kode;
					$this->app_model->insertData("skbm",$up);
