...
class Sktkt extends CI_Controller {

	/**
	 * @author : Hartanto Kurniawan,S.Kom
	 * @keterangan : Controller untuk halaman sktkt
	 **/
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			if($cari!=''){
				$where = " WHERE nmr_surat LIKE '%$cari%' OR sktkt_nama LIKE '%$cari%' OR nik LIKE '%$cari%'";
			}else if($cari_tgl!=''){
			...
			$d['judul']="Surat Keterangan Tidak Keberatan Dari Tetangga";
			...
			$text = "SELECT * FROM sktkt $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/sktkt/index/';
			...
			$d['content'] = $this->load->view('sktkt/view', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Tambah Surat Keterangan Tidak Keberatan Dari Tetangga";
			...
			$d['content'] = $this->load->view('sktkt/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['judul'] = "Ubah Info Surat Keterangan Tidak Keberatan Dari Tetangga";
			
			$id = $this->uri->segment(3);
			$text = "SELECT * FROM sktkt WHERE nmr_surat='$id'";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(3);
			$this->app_model->manualQuery("DELETE FROM sktkt WHERE nmr_surat='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/sktkt'>";			
		}else{
			header('location:'.base_url());
		}
	}
	public function checkdata()
	{
		...
		if(!empty($cek)){			
			...
			$data = $this->app_model->getSelectedData("sktkt",$id);
				if($data->num_rows()<= 0){
					echo '{"success":false}'; 
				}else{
					echo '{"success":true}'; 
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['sktkt_nama']		= $this->input->post('sktkt_nama');
				...
				$up['syarat']			= $this->input->post('syarat');
                ...
                $up['nm_surat']		    = 'Surat Keterangan Tidak Keberatan Tetangga';
				...
				if($data->num_rows()>0){
					...
					$this->app_model->updateData("sktkt",$up,$id);
					$data="Update data Sukses<span style='diplay:none;>".$up['nmr_surat']."</span>";
					echo $data;	
				}else{
				$kode	= $this->app_model->MaxKodeSktkt();
				$up['nmr_surat']		= $kode;
					$this->app_model->insertData("sktkt",$up);
