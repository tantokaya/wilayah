...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		if(maksud.length==0){
			$.messager.alert('Info','Maaf, Maksud Bepergian Belum Diisi !','warning');
			$("#maksud").focus();
			return false();
		}
		if(tujuan.length==0){
			$.messager.alert('Info','Maaf, Tujuan Bepergian Belum Diisi !','warning');
			$("#tujuan").focus();
			return false();
		}
		if(lama.length==0){
			$.messager.alert('Info','Maaf, Lama Bepergian Belum Diisi !','warning');
			$("#lama").focus();
			return false();
		}
		if(turut.length==0){
			$.messager.alert('Info','Maaf, Turut Bepergian Belum Diisi !','warning');
			$("#turut").focus();
			return false();
		}
		if(barang.length==0){
			$.messager.alert('Info','Maaf, Barang Untuk Bepergian Belum Diisi !','warning');
			$("#barang").focus();
			return false();
		}
		if(kendaraan.length==0){
			$.messager.alert('Info','Maaf, Kendaraan Untuk Bepergian Belum Diisi !','warning');
			$("#kendaraan").focus();
			return false();
		}
		if(tgl.length==0){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skb/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/skb/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/skb_pdf/'+nmr_surat);
			}else{  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skb/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skb/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skb/kecamatan",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="skb_nama" id="bio_nama"  size="50" class="detail" maxlength="50" value="<?php echo $skb_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
    ...
    <table width="100%">
	<tr>    
        <td>Maksud Bepergian</td>
        <td>:</td>
        <td><textarea name="maksud" id="maksud" style="width:320px; height:80px;" data-options="required:true"  class="detail" /><?php echo $maksud;?></textarea></td>
    </tr>
	<tr>    
        <td>Tempat yang dituju</td>
        <td>:</td>
        <td><textarea name="tujuan" id="tujuan" style="width:320px; height:80px;" data-options="required:true"  class="detail" /><?php echo $tujuan;?></textarea></td>
    </tr>
	<tr>    
        <td>Lamanya berpergian</td>
        <td>:</td>
        <td><textarea name="lama" id="lama" style="width:320px; height:80px;" data-options="required:true"  class="detail" /><?php echo $lama;?></textarea></td>
    </tr>
	<tr>    
        <td>Yang turut dalam berpergian</td>
        <td>:</td>
        <td><textarea name="turut" id="turut" style="width:320px; height:80px;" data-options="required:true"  class="detail" /><?php echo $turut;?></textarea></td>
    </tr>
	<tr>    
        <td>Barang-barang yang dibawa</td>
        <td>:</td>
        <td><textarea name="barang" id="barang" style="width:320px; height:80px;" data-options="required:true"  class="detail" /><?php echo $barang;?></textarea></td>
    </tr>
	<tr>    
        <td>Memperguanakan kendaraan</td>
        <td>:</td>
        <td><textarea name="kendaraan" id="kendaraan" style="width:320px; height:80px;" data-options="required:true"  class="detail" /><?php echo $kendaraan;?></textarea></td>
    </tr>
	<tr>    
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/skb/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/skb/">
