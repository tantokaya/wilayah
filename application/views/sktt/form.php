...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		if(nama.length==0){
			$.messager.alert('Info','Maaf, Nama Lengkap Belum di Pilih !','warning');
			$("#bio_nama").focus();
			return false();
		}
		if(tmp.length==0){
		...
		if(ttd_jenis.length!=0 && ttd_nama.length==0){
			$.messager.alert('Info','Maaf, Nama Penandatangan Belum di Pilih !','warning');
			$("#ttd_nama").focus();
			return false();
		}
		if(tgl.length==0){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/sktt/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktt/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/sktt_pdf/'+nmr_surat);
			}else{  
	...
	var ttd_jenis = $("#ttd_jenis").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktt/ttd",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktt/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktt/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktt/kecamatan",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="sktt_nama" id="bio_nama"  size="50" class="detail" maxlength="50" value="<?php echo $sktt_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/sktt/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/sktt/">
