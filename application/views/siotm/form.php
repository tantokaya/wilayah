...
//////////// NIK CALON PASANGAN ////////////	
	$("#nik4").keyup(function(e){
		var isi = $(e.target).val();
		$(e.target).val(isi.toUpperCase());
	});
	$("#nik4").focus(function(e){
		var isi = $(e.target).val();
		CariNik4();
	});
	...
	function CariNik4(){
		...
		$.ajax({
			...
			dataType : "json",
			success	: function(data){
				$("#no_kk4").val(data.no_kk);
				$("#bio_nama4").val(data.bio_nama);
				$("#bio_tmp_lahir4").val(data.bio_tmp_lahir);
				$("#bio_tgl_lahir4").val(data.bio_tgl_lahir);
				$("#bio_jk4").val(data.bio_jk);
				$("#bio_kew4").val(data.bio_kew);
				$("#bio_agama4").val(data.bio_agama);
				$("#bio_pekerjaan4").val(data.bio_pekerjaan);
				$("#bio_alamat4").val(data.bio_alamat);
				$("#bio_rt4").val(data.bio_rt);
				$("#bio_rw4").val(data.bio_rw);
				$("#bio_propinsi4").val(data.bio_propinsi);
				$("#kota4").html(data.bio_kota);
				$("#bio_kecamatan4").html(data.bio_kecamatan);
				$("#bio_kelurahan4").html(data.bio_kelurahan);
				$("#bio_kodepos4").val(data.bio_kodepos);
			}
		});
	};
	...
	$("#simpan").click(function(){
		...
		var nik4	= $("#nik4").val();
		var nama4 = $("#bio_nama4").val();
		var tgl_lahir4 = $("#bio_tgl_lahir4").val();
		var tmp_lahir4 = $("#bio_tmp_lahir4").val();
		var jk4 = $("#bio_jk4").val();
		var kew4 = $("#bio_kew4").val();
		var agama4 = $("#bio_agama4").val();
		var pek4 = $("#bio_pekerjaan4").val();			
		var kodepos4 = $("#bio_kodepos4").val();
		var alamat4 = $("#bio_alamat4").val();
		var propinsi4 = $("#bio_propinsi4").val();
		var kota4 = $("#kota4").val();
		var kecamatan4 = $("#bio_kecamatan4").val();
		var kelurahan4 = $("#bio_kelurahan4").val();
		...
		if(nik4.length==0 || nik4.length<16){
			$.messager.alert('Info','Maaf, Kode NIK Calon Pasangan tidak lengkap !','warning');
			$("#nik4").focus();
			return false();
		}
		if(nama4.length==0){
			$.messager.alert('Info','Maaf, Nama Lengkap Calon Pasangan Belum Diisi !','warning');
			$("#bio_nama4").focus();
			return false();
		}
		if(tmp_lahir4.length==0){
			$.messager.alert('Info','Maaf, Tempat Lahir Calon Pasangan Belum Diisi !','warning');
			$("#bio_tmp_lahir4").focus();
			return false();
		}
		if(tgl_lahir4.length==0){
			$.messager.alert('Info','Maaf, Tanggal Lahir Calon Pasangan Belum Diisi !','warning');
			$("#bio_tgl_lahir4").focus();
			return false();
		}
		if(jk4.length==0){
			$.messager.alert('Info','Maaf, Jenis Kelamin Calon Pasangan Belum di Pilih !','warning');
			$("#bio_jk4").focus();
			return false();
		}
		if(kew4.length==0){
			$.messager.alert('Info','Maaf, Kewarganegaraan Calon Pasangan Belum di Pilih !','warning');
			$("#bio_kew4").focus();
			return false();
		}
		if(pek4.length==0){
			$.messager.alert('Info','Maaf, Pekerjaan Calon Pasangan Belum di Pilih !','warning');
			$("#bio_pek4").focus();
			return false();
		}
		if(agama4.length==0){
			$.messager.alert('Info','Maaf, Agama Calon Pasangan Belum di Pilih !','warning');
			$("#bio_agama4").focus();
			return false();
		}
		if(propinsi4.length!=0 && kota4.length==0){
			$.messager.alert('Info','Maaf, Nama Kota Calon Pasangan Belum di Pilih !','warning');
			$("#bio_kota").focus();
			return false();
		}
		if(kota4.length!=0 && kecamatan4.length==0){
			$.messager.alert('Info','Maaf, Nama Kecamatan Calon Pasangan Belum di Pilih !','warning');
			$("#bio_kecamatan").focus();
			return false();
		}
		if(kecamatan4.length!=0 && kelurahan4.length==0){
			$.messager.alert('Info','Maaf, Nama Kelurahan Calon Pasangan Belum di Pilih !','warning');
			$("#bio_kelurahan4").focus();
			return false();
		}
		if(alamat4.length==0){
			$.messager.alert('Info','Maaf, Alamat Calon Pasangan Belum Diisi !','warning');
			$("#bio_alamat4").focus();
			return false();
		}
		if(kodepos4.length>0 && kodepos4.length<5){
			$.messager.alert('Info','Maaf, Kodepos Calon Pasangan tidak lengkap !','warning');
			$("#bio_kodepos4").focus();
			return false();
		}
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/siotm/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
...
///////////// CARI CALON PASANGAN ////////////	
	...
	$("#cari_nik4").click(function(){
		AmbilDaftarNik4();
		$("#dlg4").dialog('open');
		
	});
	
	$("#text_cari4").keyup(function(){
		AmbilDaftarNik4();
	});
	
	function AmbilDaftarNik4(){
		var cari4 = $("#text_cari4").val();
		
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/ref_json/DataNik4",
			data	: "cari4="+cari4,
			cache	: false,
			success	: function(data){
				$("#daftar_nik4").html(data);
			}
		});
	}
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/siotm/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/siotm_pdf/'+nmr_surat);
			}else{  
	...
	$("#bio_propinsi4").change(function(){  
		AmbilPropinsi4();
	});
	function AmbilPropinsi4(){  
	var propinsi = $("#bio_propinsi4").val();
	var nik = $("#nik4").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/siotm/propinsi",  
			data:{propinsi: propinsi,nik:nik},
			...
			success:function(data){  
			$("#bio_kota4").html(data);
			$("#bio_kecamatan4").html("<option value=''>-PILIH-</option>");
			$("#bio_kelurahan4").html("<option value=''>-PILIH-</option>");
			}
		});  
	}

	$("#bio_kota4").change(function(){  
		AmbilKota4();
	});

	function AmbilKota4(){  
	var kota = $("#bio_kota4").val();
	var propinsi = $("#bio_propinsi4").val();
	var nik = $("#nik4").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/siotm/kota",  
			...
			type:"POST", 
			success:function(data){  
			$("#bio_kecamatan4").html(data);
			$("#bio_kelurahan4").html("<option value=''>-PILIH-</option>");
			}
		});  
	}

	$("#bio_kecamatan4").change(function(){  
		AmbilKecamatan4();
	});

	function AmbilKecamatan4(){  
	var kecamatan = $("#bio_kecamatan4").val();
	var kota = $("#bio_kota4").val();
	var propinsi = $("#bio_propinsi4").val();
	var nik = $("#nik4").val();
		$.ajax({  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="siotm_nama1" id="bio_nama1"  size="50" class="detail" maxlength="50"  value="<?php echo $siotm_nama1;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="siotm_nama2" id="bio_nama2"  size="50" class="detail" maxlength="50" value="<?php echo $siotm_nama2;?>"/></td>
    </tr>
	<tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
	...
	<tr>    
        <td width="150">No KK</td>
        <td>:</td>
        <td><input type="text" name="no_kk3" id="no_kk3" size="18" maxlength="18" class="easyui-validatebox" data-options="required:true,validType:'length[3,18]'" value="<?php echo $no_kk3;?>"  />
        </td>
    </tr>
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="siotm_nama3" id="bio_nama3"  size="50" class="detail" maxlength="50" value="<?php echo $siotm_nama3;?>"/></td>
    </tr>
	<tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<tr>    
	...
	<tr><td><b>BioData Calon Pasangan</b></td></tr>
	<tr>    
        <td width="150">Kode NIK</td>
        <td width="5">:</td>
        <td><input type="text" name="nik4" id="nik4" size="18" maxlength="16" class="easyui-validatebox" data-options="required:true,validType:'length[3,18]'" value="<?php echo $nik4;?>"  />
        <button type="button" name="cari_nik4" id="cari_nik4" class="easyui-linkbutton" data-options="iconCls:'icon-search'">Cari</button>
        </td>
    </tr>
	<tr>    
        <td width="150">No KK</td>
        <td>:</td>
        <td><input type="text" name="no_kk4" id="no_kk4" size="18" maxlength="18" class="easyui-validatebox" data-options="required:true,validType:'length[3,18]'" value="<?php echo $no_kk4;?>"  />
        </td>
    </tr>
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="siotm_nama4" id="bio_nama4"  size="50" class="detail" maxlength="50" value="<?php echo $siotm_nama4;?>"/></td>
    </tr>
	<tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
        <td><input type="text" name="tmp_lahir4" id="bio_tmp_lahir4"  size="20" class="detail" maxlength="20"   value="<?php echo $tmp_lahir4;?>"/>
		&nbsp;&nbsp;&nbsp;&nbsp;Tgl Lahir &nbsp;&nbsp;&nbsp;&nbsp; : <input type="text" name="tgl_lahir4" id="bio_tgl_lahir4"   size="10"class="detail" maxlength="10" value="<?php echo $tgl_lahir4;?>"   />
		</td>
    </tr>
    <tr>    
        <td>Jenis Kelamin</td>
    ...
    <tr>    
        ...
        <td>:</td>
        <td>
		<select name="kew_id4" id="bio_kew4">
        <?php 
			if(empty($bio_kew4)){
		?>
        <option value="">-PILIH-</option>
	...
	<tr>    
        ...
        <td>:</td>
        <td>
        <select name="aga_id4" id="bio_agama4">
        <?php 
			if(empty($bio_agama4)){
		?>
        <option value="">-PILIH-</option>
	...
	<tr>    
        ...
        <td>:</td>
        <td>
		<select name="pek_id4" id="bio_pekerjaan4">
        <?php 
			if(empty($bio_pekerjaan4)){
		?>
        <option value="">-PILIH-</option>
        <?php
			}
			foreach($l_pekerjaan->result() as $t){
				if($pek_id4==$t->pek_kode){
		?>
			<option value="<?php echo $t->pek_kode;?>" selected="selected"><?php echo $t->pek_nama;?></option>
			<?php }else { ?>
			<option value="<?php echo $t->pek_kode;?>"><?php echo $t->pek_nama;?></option>
	...
	<tr>
		...
		<td>:</td>
		<td>
		<select name="propinsi4" id="bio_propinsi4" style="width:280px;">
			<?php 
				if(empty($bio_propinsi4)){
			?>
				<option value="">-PILIH-</option>
			<?php
				}
				foreach($l_propinsi4->result() as $t){
					if($propinsi4==$t->kd_propinsi){
			?>
				<option value="<?php echo $t->kd_propinsi;?>" selected="selected"><?php echo $t->nm_propinsi;?></option>
				<?php }else { ?>
				<option value="<?php echo $t->kd_propinsi;?>"><?php echo $t->nm_propinsi;?></option>
	...
	<tr>
		...
		<td>:</td>
		<td>
		<select name="bio_kota4" id="kota4" style="width:280px;">
		<?php 
				if(empty($kota4)){
			?>
				<option value="">-PILIH-</option>
		<?php
				...
				foreach($l_kota4->result() as $t){
					if($bio_kota4==$t->kd_kota){
			?>
				<option value="<?php echo $t->kd_kota;?>" selected="selected"><?php echo $t->nm_kota;?></option>
				<?php }else { ?>
				<option value="<?php echo $t->kd_kota;?>"><?php echo $t->nm_kota;?></option>
	...
	<tr>
		...
		<td>:</td>
		<td>
		<select name="kecamatan4" id="bio_kecamatan4" style="width:280px;">
			<?php 
				if(empty($bio_kecamatan4)){
			?>
				<option value="">-PILIH-</option>
			<?php
				}
				foreach($l_kecamatan4->result() as $t){
					if($kecamatan4==$t->kd_kecamatan){
			?>
				<option value="<?php echo $t->kd_kecamatan;?>" selected="selected"><?php echo $t->nm_kecamatan;?></option>
				<?php }else { ?>
				<option value="<?php echo $t->kd_kecamatan;?>"><?php echo $t->nm_kecamatan;?></option>
	...
	<tr>
		...
		<td>:</td>
		<td>
		<select name="kelurahan4" id="bio_kelurahan4" style="width:280px;">
			<?php 
				if(empty($bio_kelurahan4)){
			?>
				<option value="">-PILIH-</option>
			<?php
				}
				foreach($l_kelurahan4->result() as $t){
					if($kelurahan4==$t->kd_kelurahan){
			?>
				<option value="<?php echo $t->kd_kelurahan;?>" selected="selected"><?php echo $t->nm_kelurahan;?></option>
				<?php }else { ?>
				<option value="<?php echo $t->kd_kelurahan;?>"><?php echo $t->nm_kelurahan;?></option>
	...
	<tr>    
        <td>Alamat Calon Pasangan</td>
        <td>:</td>
        <td><textarea name="alamat4" style="width:420px; height:80px;" id="bio_alamat4" class="detail" /><?php echo $alamat4;?></textarea></td>
    </tr>	
	<tr>    
	<td>RT / RW</td>
    <td>:</td>
    <td><input type="text" name="rt4" id="bio_rt4"  size="5" maxlength="3" class="easyui-validatebox" value="<?php echo $rt4;?>"/>
	&nbsp; / &nbsp;:
    <input type="text" name="rw4" id="bio_rw4"  size="5" maxlength="3" class="easyui-validatebox" value="<?php echo $rw4;?>"/>
	</td>
</tr>
<tr>    
	<td>Kode Pos</td>
    <td>:</td>
    <td>
	<input type="text" name="kodepos4" id="bio_kodepos4"  size="10" maxlength="5" class="easyui-validatebox" value="<?php echo $kodepos4;?>"/>
	</td>
</tr>
	</table>
    </fieldset>
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/siotm/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/siotm/">
...
<div id="dlg4" class="easyui-dialog" title="Daftar Biodata Penduduk" style="width:900px;height:400px; padding:5px;" data-options="closed:true">
	Cari : <input type="text" name="text_cari4" id="text_cari4" size="50" />
	<div id="daftar_nik4"></div>
</div>
