...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		if(nop.length==0){
			$.messager.alert('Info','Maaf, NOP tidak lengkap !','warning');
			$("#nop").focus();
			return false();
		}
		if(nama.length==0){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skppsp/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/skppsp/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/skppsp_pdf/'+nmr_surat);
			}else{  
	...
	var ttd_jenis = $("#ttd_jenis").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skppsp/ttd",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="skppsp_nama" id="bio_nama"  size="50" class="detail" maxlength="50" value="<?php echo $skppsp_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
	...
	<tr>    
		...
		<td>:</td>
		<td>
		<input type="text" name="luas_tanah" id="luas_tanah"  size="10" maxlength="10" class="easyui-validatebox" value="<?php echo $luas_tanah;?>"/>
		</td>
	</tr>
	<tr>    
		<td>Luas Bangun [m<sup>2</sup>]</td>
		<td>:</td>
		<td>
		<input type="text" name="luas_bangun" id="luas_bangun"  size="10" maxlength="10" class="easyui-validatebox" value="<?php echo $luas_bangun;?>"/>
		</td>
	</tr>
	<tr>    
        <td>NOP</td>
        <td>:</td>
        <td><input type="text" name="nop" id="nop" size="18" maxlength="18" class="easyui-validatebox" value="<?php echo $nop;?>"  />
        </td>
    </tr>
	<tr>
		<td>Provinsi</td>
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/skppsp/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/skppsp/">
