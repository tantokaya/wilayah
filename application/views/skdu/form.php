...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		var bid_usaha 	= $("#bid_usaha").val();
		var bdn_hukum 	= $("#bdn_hukum").val();
		var ak_pen 	= $("#ak_pen").val();
		...
		if(npwp.length==0){
			$.messager.alert('Info','Maaf, Nomor NPWP Belum Diisi !','warning');
			$("#npwp").focus();
			return false();
		}
		if(perusahaan.length==0){
			$.messager.alert('Info','Maaf, Nama Perusahaan Belum Diisi !','warning');
			$("#perusahaan").focus();
			return false();
		}
		if(bid_usaha.length==0){
			$.messager.alert('Info','Maaf, Bidang Usaha Belum Diisi !','warning');
			$("#bid_usaha").focus();
			return false();
		}
		if(alamat_usaha.length==0){
			$.messager.alert('Info','Maaf, Alamat Usaha Belum Diisi !','warning');
			$("#alamat_usaha").focus();
			return false();
		}
		if(tgl.length==0){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skdu/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/skdu/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/skdu_pdf/'+nmr_surat);
			}else{  
	...
	function TglBerlaku(){
		var tgl_berlaku	= $("#tgl2").val();
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skdu/berlaku",
			data	: "tgl_berlaku="+tgl_berlaku,
			cache	: false,
			dataType : "json",
			success	: function(data){
				$("#tgl3").val(data.berlaku);
			}
		});
	}
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skdu/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skdu/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skdu/kecamatan",  
...
<td valign="top" width="50%">
    ...
    <tr>    
        <td>Tanggal Surat</td>
        <td>:</td>
        <td><input type="text" readonly name="tgl_surat" id="tgl"  size="10" maxlength="10" class="easyui-validatebox" data-options="required:true,validType:'length[3,10]'" value="<?php echo $tgl_surat;?>"/></td>
    </tr>
    
<tr>    
        <td width="150">Kode NIK</td>
        <td width="5">:</td>
        <td><input type="text" name="nik" id="nik" size="18" maxlength="18" class="easyui-validatebox" data-options="required:true,validType:'length[3,18]'" value="<?php echo $nik;?>"  />
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="skdu_nama" id="bio_nama"  size="50" class="detail" maxlength="50" value="<?php echo $skdu_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
        <td><input type="text" name="kota_lahir" id="bio_tmp_lahir"  size="20" class="detail" maxlength="20"  value="<?php echo $kota_lahir;?>"/>
...
<td valign="top" width="50%">
	...
	<tr>    
        <td>N P W P</td>
        <td>:</td>
        <td><input type="text" name="npwp" id="npwp"  size="30"class="detail" maxlength="28" value="<?php echo $npwp;?>"  /></td>
    </tr>
	
	<tr>    
        <td>Perusahaan</td>
        <td>:</td>
        <td><input type="text" name="perusahaan"  id="perusahaan" size="20"class="detail" maxlength="30" data-options="required:true,validType:'length[3,20]'" value="<?php echo $perusahaan;?>" /></td>
    </tr>
	<tr>    
        <td>Bidang Usaha</td>
        <td>:</td>
        <td><input type="text" name="bid_usaha" id="bid_usaha" size="20"class="detail" maxlength="20" data-options="required:true,validType:'length[3,20]'" value="<?php echo $bid_usaha;?>" /></td>
    </tr>
    <tr>    
        <td>Badan Hukum</td>
        <td>:</td>
        <td><input type="text" name="bdn_hukum" id="bdn_hukum" size="13"  maxlength="10"class="detail" data-options="required:true,validType:'length[3,50]'" maxlength="50" value="<?php echo $bdn_hukum;?>" /></td>
    </tr>
    <tr>    
        <td>Akta Pendirian</td>
        <td>:</td>
        <td><input type="text" name="ak_pen" id="ak_pen" size="40"  maxlength="25"class="detail" data-options="required:true,validType:'length[3,50]'" maxlength="50" value="<?php echo $ak_pen;?>" /></td>
    </tr>
    <tr>    
        <td>Alamat Usaha</td>
        <td>:</td>
        <td><textarea name="alamat_usaha" id="alamat_usaha" style="width:250px; height:80px;" id="bio_tgl_lahir" data-options="required:true"  class="detail" /><?php echo $alamat_usaha;?></textarea></td>
    </tr>
	<tr>    
	...
	<tr>    
        ...
        <td>:</td>
        <td><input readonly type="text" name="tgl_berlaku" id="tgl3" size="10"class="detail" maxlength="10" value="<?php echo $tgl_berlaku;?>"/></td>
    </tr>
   <tr>    
        <td>Penandatangan</td>
        <td>:</td>
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/skdu/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/skdu/">
