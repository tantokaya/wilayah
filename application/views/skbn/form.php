...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skbn/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbn/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/skbn_pdf/'+nmr_surat);
			}else{  
	...
	var ttd_jenis = $("#ttd_jenis").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbn/ttd",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbn/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbn/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbn/kecamatan",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="skbn_nama" id="bio_nama"  size="50" class="detail" maxlength="50"  value="<?php echo $skbn_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
    ...
    <table width="100%">
	<tr>    
        <td colspan="3">Menerangkan bahwa nama tersebut tercatat dengan ejaan/huruf yang berbeda yaitu</td>
    </tr>
	<tr>
		<td colspan="3"><input type="text" name="maksud1" id="maksud1" size="50"class="detail" maxlength="50" data-options="required:true,validType:'length[3,50]'" value="<?php echo $maksud1;?>" /></td>
    </tr>
	<tr>    
        <td colspan="3">Adapun yang benar adalah </td>
    </tr>
	<tr>
		<td colspan="3"><input type="text" name="maksud2" id="maksud2" size="50"class="detail" maxlength="50" data-options="required:true,validType:'length[3,50]'" value="<?php echo $maksud2;?>" /></td>
	</tr>
	<tr>    
        <td colspan="3">Yang tercantum dalam</td>
    </tr>
	<tr>
		<td colspan="3"><input type="text" name="maksud3" id="maksud3" size="50"class="detail" maxlength="50" data-options="required:true,validType:'length[3,50]'" value="<?php echo $maksud3;?>" /></td>
	</tr>
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/skbn/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/skbn/">
