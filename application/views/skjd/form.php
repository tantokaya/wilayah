...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skjd/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/skjd/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/skjd_pdf/'+nmr_surat);
			}else{  
	...
	var ttd_jenis = $("#ttd_jenis").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skjd/ttd",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skjd/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skjd/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skjd/kecamatan",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="skjd_nama" id="bio_nama"  size="50" class="detail" maxlength="50"  value="<?php echo $skjd_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/skjd/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/skjd/">
