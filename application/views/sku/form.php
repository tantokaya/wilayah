...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		var lok_usaha 	= $("#lok_usaha").val();	
		var jns_usaha 	= $("#jns_usaha").val();	
        ...
        if(jns_usaha.length==0){
			$.messager.alert('Info','Maaf, Jenis Usaha Belum Diisi !','warning');
			$("#jns_usaha").focus();
			return false();
		}
		if(lok_usaha.length==0){
			$.messager.alert('Info','Maaf, Lokasi Usaha Belum Diisi !','warning');
			$("#lok_usaha").focus();
			return false();
		}
		if(tgl.length==0){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/sku/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/sku/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/sku_pdf/'+nmr_surat);
			}else{  
	...
	function TglBerlaku(){
		var tgl_berlaku	= $("#tgl_pengajuan").val();
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/sku/berlaku",
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sku/propinsi",  
	...
	$("#bio_kota").change(function(){  
		AmbilKota();
	});

	function AmbilKota(){  
	var kota = $("#bio_kota").val();
	var propinsi = $("#bio_propinsi").val();
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sku/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sku/kecamatan",  
...
<td valign="top" width="50%">
    ...
    <tr>    
        <td>Tanggal Surat</td>
        <td>:</td>
        <td><input type="text" name="tgl_surat" id="tgl"  size="10" maxlength="10" readonly class="easyui-validatebox" data-options="required:true,validType:'length[3,10]'" value="<?php echo $tgl_surat;?>"/></td>
    </tr>
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="sku_nama" id="bio_nama"  size="50" class="detail" maxlength="50"  value="<?php echo $sku_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
	...
	<tr>    
        <td>Jenis Usaha</td>
        <td>:</td>
        <td><input id="jns_usaha" type="text" name="jns_usaha"  size="30"class="detail" maxlength="50" data-options="required:true,validType:'length[3,30]'" value="<?php echo $jns_usaha;?>" /></td>
    </tr>
	<tr>    
        <td>Lokasi Usaha</td>
        <td>:</td>
        <td><textarea id="lok_usaha" name="lok_usaha" style="width:320px; height:80px;" data-options="required:true" class="detail" /><?php echo $lok_usaha;?></textarea></td>
    </tr>
	<tr>    
        <td>Tanggal Pengajuan</td>
        <td>:</td>
        <td><input type="text" id="tgl_pengajuan" name="tgl_pengajuan" size="10"class="detail" maxlength="10" value="<?php echo $tgl_pengajuan;?>"/></td>
    </tr>
	<tr>    
