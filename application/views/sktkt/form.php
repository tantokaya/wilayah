...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		if(syarat.length==0){
			$.messager.alert('Info','Maaf, Persyaratan tidak boleh kosong !','warning');
			$("#syarat").focus();
			return false();
		}
		if(tgl.length==0){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/sktkt/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktkt/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/sktkt_pdf/'+nmr_surat);
			}else{  
	...
	var ttd_jenis = $("#ttd_jenis").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktkt/ttd",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktkt/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktkt/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/sktkt/kecamatan",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="sktkt_nama" id="bio_nama"  size="50" class="detail" maxlength="50" value="<?php echo $sktkt_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
    ...
    <table width="100%">
	<tr>    
        <td>Sebagai Persyaratan</td>
        <td>:</td>
        <td><input type="text" name="syarat" id="syarat" class="detail" size="25" maxlength="30" value="<?php echo $syarat; ?>" /></td>
    </tr>
	<tr>    
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/sktkt/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/sktkt/">
