...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skbm/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbm/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/skbm_pdf/'+nmr_surat);
			}else{  
	...
	var ttd_jenis = $("#ttd_jenis").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbm/ttd",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbm/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbm/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skbm/kecamatan",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="skbm_nama" id="bio_nama"  size="50" class="detail" maxlength="50"  value="<?php echo $skbm_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
	...
	<tr>    
        ...
        <td>:</td>
        <td><textarea name="keperluan" id="keperluan" style="width:320px; height:80px;" id="keperluan"  class="detail" /><?php echo $keperluan;?></textarea></td>
    </tr>
	<tr>    
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/skbm/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/skbm/">
