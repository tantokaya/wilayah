...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		if(keperluan.length==0){
			$.messager.alert('Info','Maaf, Keperluan Belum Diisi !','warning');
			$("#keperluan").focus();
			return false();
		}
		if(tgl.length==0){
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/skpck/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	var nmr_surat	= $("#nmr_surat").val();
		...
		$.ajax({  
			url:"<?php echo site_url(); ?>/skpck/checkdata",  
			data:"nmr_surat="+nmr_surat, 
			type:"POST", 
			dataType : "json",
			success:function(data){  
			if(data.success){  
				window.open('<?php echo site_url();?>/cetak/skpck_pdf/'+nmr_surat);
			}else{  
	...
	var ttd_jenis = $("#ttd_jenis").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skpck/ttd",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skpck/propinsi",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skpck/kota",  
	...
	var nik = $("#nik").val();
		$.ajax({  
			url:"<?php echo site_url(); ?>/skpck/kecamatan",  
...
<tr>    
    ...
    <tr>    
        <td>Nama Lengkap</td>
        <td>:</td>
        <td><input type="text" name="skpck_nama" id="bio_nama"  size="50" class="detail" maxlength="50"  value="<?php echo $skpck_nama;?>"/></td>
    </tr>
    <tr>    
        <td>Tempat Lahir</td>
        <td>:</td>
...
<td valign="top" width="50%">
	...
	<tr>    
        <td>Keperluan</td>
        <td>:</td>
        <td><textarea name="keperluan" id="keperluan" style="width:320px; height:80px;"  class="detail" /><?php echo $keperluan;?></textarea></td>
    </tr>
	<tr>    
...
<tr>
	...
	<a href="<?php echo base_url();?>index.php/skpck/tambah">
    ...
    <a href="<?php echo base_url();?>index.php/skpck/">
